...
export default async function CaseManagerParticipantIntakePage(props: { params: Promise<{ id: string }> }) {
  ...
  if (!profileSnap.exists) redirect("/case-manager");

  const profile = profileSnap.data() as any;
  const assignedCaseManagerId = typeof profile?.caseManagerId === "string" ? profile.caseManagerId : null;
  if (!assignedCaseManagerId || assignedCaseManagerId !== session.uid) redirect("/case-manager");
