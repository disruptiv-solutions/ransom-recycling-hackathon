...
import { AlertTriangle, ArrowRight, Briefcase, Kanban, LayoutGrid, MessageSquare, Plus, Recycle, Search } from "lucide-react";
        ...
        <div className="flex items-center gap-4 md:gap-6">
          <div className="relative h-12 w-12 md:h-16 md:w-16">
            <svg className="h-full w-full -rotate-90 transform" viewBox="0 0 100 100">
              <circle className="text-slate-100 dark:text-slate-800" strokeWidth="8" stroke="currentColor" fill="transparent" r={radius} cx="50" cy="50" />
              <circle
            ...
            <span className="absolute inset-0 flex items-center justify-center text-[10px] font-bold md:text-xs">{percentage}%</span>
          </div>
          <div>
...
const ParticipantCard = ({ id, name, checkIn, phase, color, avatar }: { id: string; name: string; checkIn: string; phase: string; color: "mint" | "amber"; avatar: string }) => (
  <Card className="group relative min-w-[280px] rounded-card border-none bg-white p-6 shadow-sm dark:bg-slate-900 md:shadow-lg md:shadow-slate-200/40">
    ...
    <div className="mt-6 flex items-center justify-between">
      <span className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase ${color === "mint" ? "bg-mint/10 text-primary" : "bg-amber/10 text-amber-700"}`}>{phase}</span>
      <div className="flex gap-2">
        <Button variant="ghost" size="icon" asChild className="h-10 w-10 rounded-full bg-slate-50 text-slate-400 transition-all hover:bg-primary hover:text-white dark:bg-slate-800">
          <Link href={`/case-manager/participants/${id}`} aria-label={`Open ${name} profile`}>
            <ArrowRight className="h-4 w-4" />
          </Link>
        </Button>
        <button
          className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-50 text-slate-400 transition-all group-hover:bg-primary group-hover:text-white dark:bg-slate-800"
          type="button"
...
export default async function CaseManagerHomePage() {
  const profile = await getSessionProfile();
  if (!profile) redirect("/login");
  if (profile.role !== "case_manager" && profile.originalRole !== "admin") redirect("/");
    ...
    <div className="relative flex flex-col gap-6 md:gap-8">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 md:text-base">Welcome back, {profile.displayName?.split(" ")[0] || "there"}.</p>
        </div>

        <div className="flex items-center gap-4">
          <Tabs id="case-manager-home-nav" defaultValue="dashboard" className="hidden md:flex">
          ...
          <div className="hidden items-center gap-4 md:flex">
            <div className="relative">
              ...
              <Input placeholder="Search records..." className="w-64 rounded-full border-none bg-white pl-12 shadow-sm focus:ring-2 focus:ring-primary dark:bg-slate-900" />
            </div>
            <NotificationPopover />
            <Avatar className="h-12 w-12 border-2 border-primary">
              <AvatarFallback>{(profile.displayName || "CM").substring(0, 2).toUpperCase()}</AvatarFallback>
            </Avatar>
          </div>
        </div>
      </header>

      <Tabs id="case-manager-home-main" defaultValue="dashboard" className="w-full">
          ...
          <div className="flex flex-col gap-8">
            ...
            <section>
              <div className="mb-4 flex items-center justify-between md:mb-6">
                ...
                <Link href="/case-manager/directory" className="flex items-center gap-1 text-[10px] font-bold text-primary md:text-sm md:font-semibold" aria-label="View all participants">
                  View all <ArrowRight className="h-3 w-3 md:h-4 md:w-4" />
                </Link>
              </div>
              <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide md:gap-6">
                <ParticipantCard id="1" name="Marcus Robinson" checkIn="2 days ago" phase="STABLE" color="mint" avatar="https://api.dicebear.com/7.x/avataaars/svg?seed=Marcus" />
                <ParticipantCard id="2" name="Elena Rodriguez" checkIn="1 week ago" phase="ENTRY" color="amber" avatar="https://api.dicebear.com/7.x/avataaars/svg?seed=Elena" />
              </div>
            </section>
            ...
            <div className="flex flex-col gap-6 lg:grid lg:grid-cols-3 lg:gap-8">
              ...
              <Card className="rounded-card border-none bg-primary p-6 text-white shadow-xl md:p-8">
                ...
                <Button className="mt-6 h-12 w-full rounded-full bg-white text-xs font-bold text-primary hover:bg-white/90">Review All Requests (5)</Button>
              </Card>
            </div>
          </div>
        </TabsContent>
        ...
        <TabsContent value="pipeline" className="mt-0 outline-none">
          <PhaseBoard initialParticipants={MOCK_PARTICIPANTS} basePath="/case-manager/participants" />
        </TabsContent>
      </Tabs>

      <div className="rounded-xl bg-slate-200/50 p-4 text-center text-[10px] text-slate-500 md:text-left">
        <p>
          <span className="font-bold uppercase text-primary">Program Tip</span> Regular check-ins improve retention by 40%.
        </p>
      </div>

      <Button className="fixed bottom-24 right-6 h-14 w-14 rounded-full bg-primary p-0 shadow-lg md:hidden" aria-label="Create quick action">
